...
- プロジェクトでは [oxlint](https://oxc.rs/) によるリンティングと [oxfmt](https://oxc.rs/docs/guide/usage/formatter.html) によるフォーマットを行っています。コミット前に `pnpm lint:fix` を実行して、コードスタイルを統一してください。
  主要なルールは `.oxlintrc.json` と `.oxfmtrc.json` で設定されています。
