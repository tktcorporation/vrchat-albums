...
#
# スコープ: ガード対象は Write/Edit/MultiEdit（エージェントの主たる編集経路）のみ。
# Bash 経由の書き込み（sed -i / リダイレクト等）は意図的に対象外とする。完全な
# サンドボックスではなく「設定を緩める安易な逃げ」を塞ぐスピードバンプであり、Bash
# まで広げるとヒューリスティック回避や正当な Bash 操作の誤ブロックを招くため。
set -euo pipefail
