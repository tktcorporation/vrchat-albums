...
<claude-instructions>
  ...
  <linter-improvement-tracking jp="Linter化候補の記録">
    <directory>./issues/</directory>
    <purpose>CLAUDE.mdに書かれた規約のうち、linterとして実装すべきものを記録</purpose>
    <file-format>
      <naming>{YYYYMMDD}-{short-description}.md</naming>
      <template>issues/README.md参照</template>
    </file-format>
    <implementation-options>
      <option name="TypeScript Compiler API" when="型情報が必要な複雑なチェック">
        scripts/lint-*.ts として実装
      </option>
      <option name="GritQL" when="シンプルなパターンマッチング">
        .grit/patterns/*.md として実装
      </option>
      <note>ESLintは使わない（Biome移行済み）</note>
    </implementation-options>
    <when-to-create>
      <trigger>規約違反を手動でチェックしている場合</trigger>
      <trigger>同じ指摘を複数回している場合</trigger>
      <trigger>自動検出が技術的に可能な場合</trigger>
    </when-to-create>
  </linter-improvement-tracking>
