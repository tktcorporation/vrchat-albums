...
test('初期ユーザーフロー: 規約同意 → セットアップ → ギャラリー表示', async ({
  ...
  // 写真カードが表示されるのを待つ（サムネイル読み込み含む）
  // LocationGroupHeader はログ同期後すぐ表示されるが、
  // 写真カードはインデックス処理 + サムネイル生成後に描画される。
  const photoCardCount = await page.locator('.photo-card').count();
  console.log(`[Step 3] Photo cards found: ${photoCardCount}`);
  if (photoCardCount === 0) {
    // 写真がまだ表示されていない場合、追加で待機
    console.log('[Step 3] Waiting for photo cards to render...');
    await page.waitForSelector('.photo-card', { timeout: 30_000 });
    const updatedCount = await page.locator('.photo-card').count();
    console.log(`[Step 3] Photo cards after wait: ${updatedCount}`);
  }
