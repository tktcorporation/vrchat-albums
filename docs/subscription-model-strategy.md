...
※ Polar.sh 手数料（約6%）/ LemonSqueezy 手数料（約8%）控除前

**参考**: Discord Nitro は 2025年に $561M（約840億円）の売上を達成し、前年比29.2%成長。フリーミアムからの課金転換が成功している代表例。
...
| **買い切りのみ** | 継続的な開発資金が確保できない。ただしハイブリッド（買い切り+有料アップデート）は検討余地あり（後述） |
...
#### A. 買い切り + 有料メジャーアップデート（Transmit/Sublime Text 型）

- 初回: ¥2,000〜3,000 の買い切り
- メジャーバージョンごとに再購入（¥1,500〜2,000、既存ユーザー割引）
- **メリット**: VRChat コミュニティの「無料ツール文化」への抵抗が少ない。一回の決済で完結
- **デメリット**: 収益が不安定。メジャーバージョンのリリースサイクルに依存
- **参考**: CleanShot X ($29)、Transmit 5 ($45)

#### B. サブスク + パーペチュアルフォールバック（JetBrains 型）

- サブスク解約後も、支払い期間中にリリースされたバージョンは永続利用可能
- **メリット**: サブスク疲れへの対策。「お金を払った分は残る」安心感
- **デメリット**: 実装が複雑。バージョン管理の負荷
- **参考**: JetBrains IDEs、Sketch

#### C. ハイブリッド（推奨の発展形）

Phase 1 でサブスクを導入後、ユーザーの反応を見て買い切りオプションを追加:
- サブスク: ¥400/月（常に最新機能）
- 買い切り: ¥5,000（購入時点の Pro 機能を永続利用、1年間のアップデート付き）
