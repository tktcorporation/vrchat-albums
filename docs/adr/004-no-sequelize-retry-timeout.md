...
**`configureSQLitePragmas` 内の `client.query('PRAGMA ...')` および各モデルの `sync()` は DBQueue を経由しない**ため、ここで詰まった場合の自動検知機構は存在せず、ユーザーがアプリを強制終了するまでブロックする。SQLite 単プロセス DB の性質上、これらの段階で長時間ハングするケースは現実的に発生しないと判断している。
